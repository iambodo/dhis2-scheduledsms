...
packages<-c("openair","ggplot2","dplyr","stringr","lubridate","httr","assertthat","readr","stats","graphics", "grdevices","utils","datasets","DT","methods","base","knitr")      
install_or_load_pack <- function(pack){
  create.pkg <- pack[!(pack %in% installed.packages()[, "packages"])]
  if (length(create.pkg))
    install.packages(create.pkg, dependencies = TRUE)
  sapply(pack, require, character.only = TRUE)
}
####IMPORTANT!!! before executing "Knit" command, ***Uncomment this funtion***, then run this chunk in console :
#install_or_load_pack(packages)
####THEN, comment out or delete above command
...
DT::datatable(sent_sms, escape = FALSE)
sort(summary(as.factor(sched_sms$msg_name)), decreasing = TRUE)
sort(summary(as.factor(sched_events$orgUnit)))
